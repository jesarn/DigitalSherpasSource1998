...
'----weekend------
if day(date) = 6 and hour(time) = 23 and minute(time) > 34 then %>
     <STRONG>Eyewitness News</STRONG> tomorrow night at 6pm
<% elseif day(date) = 7 and hour(time) < 18 then %>
     <STRONG>Eyewitness News</STRONG> tonight at 6pm
<% elseif day(date) = 7 and hour(time) = 18 and minute(time) < 30 then %>
     <STRONG>Eyewitness News</STRONG> right now
<% elseif day(date) = 7 and hour(time) = 18 then %>
     <STRONG>Eyewitness News</STRONG> tonight at 11pm 
<% elseif day(date) = 7 and hour(time) > 18 and hour(time) < 23 then %>
     <STRONG>Eyewitness News</STRONG> tonight at 11pm 
<% elseif day(date) = 7 and hour(time) = 23 and minute(time) < 30 then %>
     <STRONG>Eyewitness News</STRONG> right now
<% elseif day(date) = 7 and hour(time) = 23 then %>
     <STRONG>Eyewitness News</STRONG> tomorrow night at 6pm

<% elseif day(date) = 1 and hour(time) < 18 then %>
     <STRONG>Eyewitness News</STRONG> tonight at 6pm
<% elseif day(date) = 1 and hour(time) = 18 and minute(time) < 30 then %>
     <STRONG>Eyewitness News</STRONG> right now
<% elseif day(date) = 1 and hour(time) = 18 then %>
     <STRONG>Eyewitness News</STRONG> tonight at 11pm 
<% elseif day(date) = 1 and hour(time) > 18 and hour(time) < 23 then %>
     <STRONG>Eyewitness News</STRONG> tonight at 11pm 
<% elseif day(date) = 1 and hour(time) = 23 and minute(time) < 30 then %>
     <STRONG>Eyewitness News</STRONG> right now
<% elseif day(date) = 1 and hour(time) = 23 then %>
...
<a href="http://207.137.189.177/cgi-bin/Webdriver?MIval=Mainmenu1&linked=1&affil_name=KION&city=15&net=2" target="_top"><center><img src="images/chattvlogo2.gif" width=150 height=56 alt="ChatTV" border=0></center></a><br>
      ...
      <TD class=a ALIGN=LEFT VALIGN=TOP>
	     ...
	     <A HREF="ncaa/default.html" TARGET="_top"><IMG SRC="images/ads/w_ncaa.gif" WIDTH=125 HEIGHT=60 BORDER=0 ALT="NCAA"></A>
             </TD>
           </TR>
           <TR>
		 ...
		 <P><a href="<!--#include file="dailylink.inc"-->" target="main"><!--#include file="dailyshow.inc"--></P>
		 </center>
