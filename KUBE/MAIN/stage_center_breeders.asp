...
<% If weekday(date) = 1 or weekday(date) = 7 then %>
	<param NAME="Movie" VALUE="../flash/kube_audio.spl">
<% Else  %>
	...
	<% Elseif hour(time) >=  4 and hour(time) <= 5 then %>
		<param NAME="Movie" VALUE="../flash/assup.spl">
	<% Elseif hour(time) >=  6 and hour(time) <= 9 then %>
		<param NAME="Movie" VALUE="../flash/breeders_cup.swf">
	<% Elseif hour(time) >=  10 and hour(time) <= 11 then %>
		<param NAME="Movie" VALUE="../flash/newmusic.spl">
	<% Elseif hour(time) =  12 then %>
		<param NAME="Movie" VALUE="../flash/skool.spl">
	<% Elseif hour(time) >=  13 and hour(time) <= 16 then %>
		<param NAME="Movie" VALUE="../flash/newmusic.spl">
	<% Elseif hour(time) =  17 then %>
		<param NAME="Movie" VALUE="../flash/5oclock.spl">
	<% Elseif hour(time) =  18 then %>
		<param NAME="Movie" VALUE="../flash/newmusic.spl">
	<% Elseif hour(time) =  19 then %>
		<param NAME="Movie" VALUE="../flash/7@7.spl">
	<% End If %>
<% End If %>
...
<% If weekday(date) = 1 or weekday(date) = 7 then %>
	<embed SRC="../flash/kube_audio.spl"
<% Else  %>
	<% If hour(time) >= 0 and hour(time) <= 3 then %>
		<embed SRC="../flash/kube_audio.spl"
	<% Elseif hour(time) >= 20 and hour(time) <= 23 then %>
		<embed SRC="../flash/kube_audio.spl"
	<% Elseif hour(time) >=  4 and hour(time) <= 5 then %>
		<embed SRC="../flash/assup.spl"
	<% Elseif hour(time) >=  6 and hour(time) <= 9 then %>
		<embed SRC="../flash/breeders_cup.swf"
	<% Elseif hour(time) >=  10 and hour(time) <= 11 then %>
		<embed SRC="../flash/newmusic.spl"
	<% Elseif hour(time) =  12 then %>
		<embed SRC="../flash/skool.spl"
	<% Elseif hour(time) >=  13 and hour(time) <= 16 then %>
		<embed SRC="../flash/newmusic.spl"
	<% Elseif hour(time) =  17 then %>
		<embed SRC="../flash/5oclock.spl"
	<% Elseif hour(time) =  18 then %>
		<embed SRC="../flash/newmusic.spl"
	<% Elseif hour(time) =  19 then %>
		<embed SRC="../flash/7@7.spl"
	<% End If %>
<% End If %>
